  ...
  const formState = {
    ...
    init() {
      ...
      if (savedState) {
        ...
        this.additionalCosts.geyserCount = 0;
        // Common properties
    ...
    //           const onePercentcomm = total * 0.01;
    // const roundedRangess = Math.round(onePercentcomm / 1000) * 1000; // Rounds to nearest 1000

    // document.getElementById('commercial-lower-range').textContent = (total - roundedRangess).toLocaleString();
    // document.getElementById('commercial-higher-range').textContent = (total + roundedRangess).toLocaleString();
    ...
    updateCommercialEstimateDisplay() {
      // Format and display main estimate
      const formattedEstimate = this.currentEstimate.toLocaleString();
      document.getElementById('commercial-estimated-cost').textContent = `Rs. ${formattedEstimate}`;

      // Calculate dynamic range (1% rounded to nearest 1000)
      const onePercentRange = this.currentEstimate * 0.01;
      const roundedRange = Math.round(onePercentRange / 1000) * 1000;

      // Calculate and display ranges
      const lowerRange = (this.currentEstimate - roundedRange).toLocaleString();
      const higherRange = (this.currentEstimate + roundedRange).toLocaleString();
    ...
    restoreSelections() {
      ...
      document.getElementById('geyser-count').textContent = this.additionalCosts.geyserCount;
    ...
    //     const onePercent = basePrice * 0.01;
    //     let roundedRange;

    //     if (onePercent < 5000) {
    //       roundedRange = Math.round(onePercent / 1000) * 1000; // Round to 1000 if < 5000
    //     } else {
    //       roundedRange = Math.round(onePercent / 5000) * 5000; // Round to 5000 if ≥ 5000
    //     }

    //     document.getElementById('lower-range').textContent = (basePrice - roundedRange).toLocaleString();
    //     document.getElementById('higher-range').textContent = (basePrice + roundedRange).toLocaleString();
    //   } else {
    ...
    //       const roundedRange = Math.round(onePercent / 1000) * 1000; // Rounds to nearest 1000

    //       document.getElementById('final-lower-range').textContent = (total - roundedRange).toLocaleString();
    //       document.getElementById('final-higher-range').textContent = (total + roundedRange).toLocaleString();
    //     }
    //   }
    // },
...
calculateEstimate() {
  ...
  const calculateDynamicRange = (price) => {
    ...
    return onePercent < 5000 
      ? Math.round(onePercent / 1000) * 1000  // Round to nearest 1000 if <5000
      : Math.round(onePercent / 5000) * 5000; // Round to nearest 5000 if ≥5000
  };
  ...
  if (this.propertySize === '<6marla') {
    ...
    // Calculate and display dynamic range
    const range = calculateDynamicRange(basePrice);
    document.getElementById('lower-range').textContent = (basePrice - range).toLocaleString();
    document.getElementById('higher-range').textContent = (basePrice + range).toLocaleString();
  } else {
    // Calculate room estimate first
    this.calculateRoomEstimate();
    let total = this.currentEstimate;

    // Add additional costs based on current step
    if (this.currentStep >= 7) total += this.additionalCosts.geyserCount * this.additionalCosts.geyser;
    if (this.currentStep >= 8 && this.additionalCosts.hasWaterPump) total += this.additionalCosts.waterPump;
    if (this.currentStep >= 9 && this.additionalCosts.hasInstallation) total += this.additionalCosts.installation;

    // Update display based on current step
    const displayElements = {
      7: { id: 'geyser-estimated-cost', showRange: false },
      8: { id: 'pump-estimated-cost', showRange: false },
      9: { id: 'setup-estimated-cost', showRange: false },
      10: { id: 'final-cost', showRange: true }
    };

    if (displayElements[this.currentStep]) {
      const { id, showRange } = displayElements[this.currentStep];
      document.getElementById(id).textContent = `Rs. ${total.toLocaleString()}`;
      
      if (showRange) {
        const range = calculateDynamicRange(total);
        document.getElementById('final-lower-range').textContent = (total - range).toLocaleString();
        document.getElementById('final-higher-range').textContent = (total + range).toLocaleString();
      }
    }
  }
},
    ...
    updateGeyserCount(change) {
      ...
      let currentCount = this.additionalCosts.geyserCount || 0;

      // Calculate new value
      let newValue = currentCount + change;
    ...
    updateGeyserDisplay() {
      const count = this.additionalCosts.geyserCount || 0;
      const geyserCost = count * this.additionalCosts.geyser;
      const total = this.currentEstimate + geyserCost;
